...
const bgImage =
  "https://insight.kellogg.northwestern.edu/content/uploads/_1200x630_fit_center-center_82_none/Full_1222_Boycotts_Buycotts.jpg?mtime=1671551771";
...
const BlogEditorUpdate = () => {
  ...
  const handleSubmit = async () => {
   ...
   const transformedSections = sections.map(section => {
    if (section.type === "images") {
      // Filter content array to only include items with a valid Cloudinary URL
      const filteredContent = section.content
        .filter(item => item.url && item.url.includes("cloudinary"))
        .map(item => item.url);

      return {
        ...section,
        content: filteredContent.length > 0 ? filteredContent : section.content // Retain original content if no Cloudinary URL is found
      };
    }
    return section; // Return other types unchanged
  });

  console.log(JSON.stringify(transformedSections), 'transformed sections');
    ...
    sections.forEach((section, index) => {
      if (section.type === "images") {
        console.log(section?.content, 'api call section content')
        const hasCloudinaryUrl = section?.content.some(item =>
          typeof item.url === 'string' && item.url.startsWith('http://res.cloudinary.com')
        );
        if (!hasCloudinaryUrl) {
      ...
      const hasCloudinaryUrl = section.type === "images" && section?.content.some(item =>
        typeof item.url === 'string' && item.url.startsWith('http://res.cloudinary.com')
      );
      if (!hasCloudinaryUrl) {
    ...
    try {
      const response = await axios.put("/blog/updateBlog", formData, {
        headers: {
              ...
              <Grid item xs={12}>
                ...
                />
                        ...
                        <ArgonBox>
                          ...
                          {Array.isArray(tags) && tags.map((tag, index) => (                              <ArgonBox
                                key={index}
                                borderRadius="lg"
                                px={1.5}
                                py={0.8}
...
/>
          ...
          >
            <DeleteIcon fontSize="small" />
                       ...
                       {sections.map((section, index) => {
                        if (section.type === "images"){
                          console.log(section.content, 'sectionnnncontentttttttttttttttt'); // Log the section content here
    ...
    >
      <CardContent sx={{ flexGrow: 1, boxShadow: "none" }}>
        {section.type === "text" || section.type === "quote" ? (
          ...
          <BlogMediaMultiple
            handleImageChange={(fileList) => handleSectionImageChange(fileList, index)}
            // initialImage={
            //   section.content.length > 0 ? section.content[0].url
            //     : null
            // }   
            initialImages={
      ...
      >
        <DeleteIcon fontSize="small" />
                        ...
                        <Box sx={{ my: 2 }}>
                        <center>
                        <p style={{fontSize: 12}}>Add More Content</p>
                          </center>
                    ...
                    {(provided, snapshot) => (
                      ...
                      >
                                ...
                                <Grid item xs={12} key={ad.id}>
                                  <Card
                                    sx={{
                                      ...
                                      justifyContent: "flex-start",
                                      border:
                                        selectedAdsSidebar[index] === ad.id
                                          ? "3px solid #11CDEF"
                                          : "none",
                                      cursor: "pointer",
                                      transition:
                                    ...
                                    onClick={() =>
                                      handleSelectAdSidebar(ad, index)
                                    }
                                  >
                                    <CardMedia
                                      component="img"
                          ...
                          {section.type === "similarBlog" && (
                            <>
                              {blogs.length > 0 ? (
                                    ...
                                    <Grid item xs={12} sm={6} md={4} key={blog.id}>
                                      <Card
                                        sx={{
                                          ...
                                          paddingBottom: 0,
                                          border:
                                            selectedBlogsSidebar[index] === blog.id
                                              ? "3px solid #11CDEF"
                                              : "none",
                                          cursor: "pointer",
                                          transition: "transform 0.3s, box-shadow 0.3s",
                                        ...
                                        onClick={() =>
                                          handleSelectBlogsSidebar(blog, index)
                                        }
                                      >
                                        <CardMedia
                                          component="img"
                                        ...
                                        >
                                          ...
                                          >
                                            <span
                                              style={{ fontWeight: "bold", color: "#B73439" }}
                                            >
                                              Brand:
                                            </span>{" "}
                                            {blog?.brand}
                                ...
                                <Typography fontSize={12}>
                                  There was no blog found. Enter your brand first to select or
                                  add related blogs.
                                </Typography>
                              )}
                            </>
                        ...
                        <IconButton
                          ...
                          onClick={() => handleDeleteSectionSidebar(index)}
                        >
                          <DeleteIcon fontSize="small" />
                        </IconButton>
                        <hr
       ...
       <Box sx={{ my: 2 }}>
                          <center>
                            <p style={{fontSize: 12}}>Add Sidebar Content</p>
                          </center>
